...
const getPost = async (req,res,next) =>{
    try {
        ...
        const posts = await Post.find(query).populate("file").populate("category").populate("updatedBy","-password -recoverCode -varificationCode").skip((pageNo-1)*sizeNo).limit(sizeNo).sort({_id: -1});

        res.status(200).json({code: 200,status: true, message: "Posts Fetched Successfully",data:{posts,totalResult,totalPages,page}});
...
const getSinglePost = async (req,res,next)=>{
    try {
        ...
        const post = await Post.findById(id).populate("file").populate("category").populate("updatedBy","-password -recoverCode -varificationCode");
        if(!post){
            res.code = 404;
            throw new Error("Post Not Found");
        }
        
        res.status(201).json({code: 201,status: true, message: "Post Fetched Successfully",data:{post}})
